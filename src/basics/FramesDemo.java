...
public class FramesDemo {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// locate enter your name text filed which is in main page
		driver.findElement(By.id("name")).sendKeys("sunshine");
		Thread.sleep(3000);
		// driver focus is in main page, switch that focus to frame
		// switching to frame using index number
//		driver.switchTo().frame(0);
		// switching to frame using id attribute value
//		driver.switchTo().frame("courses-iframe");
		// switching to frame using name attribute value
//		driver.switchTo().frame("iframe-name");
		// switching to frame using web element
		// locate the frame
		WebElement frmToSwitch = driver.findElement(By.xpath("//iframe[contains(@src, 'courses')]"));
		driver.switchTo().frame(frmToSwitch);
		// now driver focus is in frame
		// locate search field in frame
		...
		// locate hide and show text field in main page
